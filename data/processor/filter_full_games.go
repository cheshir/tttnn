...
func simulateGame(game string) (result Result, err error) {
	table := NewTable()
	gameMoves := strings.Fields(game)
...
func convertExternalResultToInternal(result string) Result {
	switch result {
	case "1":
		return BlackWin
	case "0.5":
...
func convertInternalResultToExternal(result Result) string {
	switch result {
	case BlackWin:
		return "1"
	case BlackLose:
